...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='TDMS')

    ### model options
    # parser.add_argument('--method',  type=str,   required=True,      help='full model name')
    parser.add_argument('--name', type=str, default="", help='full model name')
    ...
    for i in range(len(dataset_task_list)):
        ...
        cmd = "CUDA_VISIBLE_DEVICES=%d python -W ignore test_biFVS.py \
            --name %s \
            --data_dir %s \
            --model BiFlowTransNet\
            --phase test \
            --dataset %s \
            --task %s \
            --which_epoch %d \
