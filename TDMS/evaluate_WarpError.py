...
if __name__ == "__main__":
    ...
    if opts.testName == "BiFVS":
        # metric_filename = os.path.join(output_dir, "proc_WarpError.txt")
        metric_filename = "./results/Our/softFlow/Sf_%s_%s_%s.txt" % (
    ...
    if not os.path.isdir("./results/Our/softFlow/"):
        os.makedirs("./results/Our/softFlow/")
    ...
    for v in range(len(video_list)):
        ...
        occ_dir = os.path.join("", "fw_occlusion", opts.dataset, video)
        flow_dir = os.path.join("", "fw_flow", opts.dataset, video)

        if opts.testName == "ECCV":
            frame_dir = os.path.join("", opts.task,
                opts.dataset, video)
        elif opts.testName == "Sig":
            frame_dir = os.path.join("",opts.task, opts.dataset, video)
        elif opts.testName == "Pro":
            frame_dir = os.path.join("", opts.task, opts.dataset, video)
