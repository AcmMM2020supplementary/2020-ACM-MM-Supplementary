...
if __name__ == "__main__":
    ...
    model_pf = networks.__dict__[opts.model](opts, nc_in=12, nc_out=3)
    ...
    model_pf = util.load_model(model=model_pf, dirc="pf", opts=opts, epoch=epoch_st)
    ...
    for v in range(len(video_list)):
        ...
        for t in range(1, len(frame_list)):
            ...
            with torch.no_grad():
                ...
                model_pf(inputs)
                output = model_pf.computeLoss(FlowNet)
                if opts.model =="SingleTDMSNet":
                    output = frame_p2 + output
                # model_pf.saveInnerInfo()
                te = time.time()
                times.append(te - ts)
